...
const getCredentials = request => {
  ...
  if(!request.headers.authorization || request.headers.authorization === "") { return null; }
  if(request.headers.authorization.split(" ")[0] !== "Basic") { return null; }

  const info = Buffer.from(request.headers.authorization.split(" ")[1], 'base64').toString('ascii');
...
const acceptsJson = request => {
  ...
  if(request.headers.accept.includes("application/json")) { return true; }
  if(request.headers.accept.includes("*/*")) { return true; }
...
const isJson = request => {
  ...
  if(!request.headers['content-type']) { return false; }
